...
```
此时不再成立，因此跳出循环去等待队列去任务执行。之后再次竞争互斥锁、继续等待。  
...
```
成立，此时被唤醒的线程会被终止。  
**4.** 管理者线程的执行流程：每隔一段时间查看线程池状态（需要通过互斥量保证线程安全），当工作线程数目不够的时候就创建线程、空闲线程数目较多的时候就销毁线程。  
```
while(!pool->shutdown) {
    sleep(3); //时间间隔
...
本项目中，如果主线程提供足够的延时，那么“管理者线程”（代码在“**4.**”中）会销毁空闲线程直至最少数量：
```
if(live > 2 * busy && live > pool->minNum) { //空闲线程较多 销毁线程 每次销毁NUMBER个 也就是2个
    pthread_mutex_lock(&pool->poolMutex);
    pool->exitNum = NUMBER;
...
```
也可能存在：主线程没有提供足够的延时，“管理者线程”销毁的线程数量不多这样一种情况。  
无论是哪种，都可以通过“存活的线程数量”来销毁剩余线程：
```
